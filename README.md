...
```bash
# Required tools
python >= 3.11   # use a venv on PEP 668–managed distros: python3 -m venv .venv && source .venv/bin/activate
terraform >= 1.6
aws-cli >= 2.x
docker >= 24.x
...
- [x] Core ETL pipeline (ingest → preprocess → load)
- [x] VLM classification engine with confidence gating
- [x] Open source corpus ingestion (CompCars + CarDD + Kaggle + HuggingFace)
- [x] Label normalization across 5 dataset schemas
- [x] Redshift schema + DynamoDB index
- [x] Terraform IaC (modular)
- [x] CloudWatch observability + X-Ray tracing
